...
func createPod(path string, timeout int, f *framework.Framework) (*v1.Pod, error) {
	app := &v1.Pod{}
	...
	app, err := f.ClientSet.CoreV1().Pods(f.UniqueName).Create(context.TODO(), app, metav1.CreateOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to create app: %w", err)
	}

	err = waitForPodInRunningState(app.Name, f.UniqueName, f.ClientSet, timeout, noError)
	if err != nil {
		return nil, fmt.Errorf("failed to wait for app running: %w", err)
	}
	return app, nil
}
